...
**ทำได้:** ตรวจ audit log, ดูข้อมูล Teacher/Student ทั้งหมด, import Teacher ผ่าน CSV, reset password ของ Teacher, export log, moderate (ลบ) Comment ทั้ง CLASS_WIDE และ PRIVATE scope (PRIVATE = Critical-tier audit · Phase 6 / ADR-0021 sibling decision)
...
- **User Feed (Phase 7):** activity รวมจากทุก active-term CourseOffering ของ User — แสดงเป็น section ในหน้า Dashboard · multi-query union ที่ application layer (ดู ADR-0023) · pagination = infinite scroll 20 ต่อ batch
- **Course Feed:** **Deferred → Phase 8.** Phase 7 เลือก hybrid surface: User Feed บน dashboard + Bell ใน navbar · ไม่มี per-course Feed tab เพราะ Assignment/Material/Announcement tab แยกตาม content-type อยู่แล้ว (ADR-0023 § Course Feed deferral)
...
- Trigger ที่ set readAt:
  1. **Click bell row** → server action `markNotificationRead(id)` · in-tx update + navigate
  2. **Mark all read button** → bulk update ทุก row ของ user ที่ `readAt IS NULL AND suppressedAt IS NULL`
- **ไม่** mark read ตอน open dropdown (Q10 = A · UX standard: เปิดดูยังไม่ใช่อ่าน)
