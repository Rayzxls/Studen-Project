...
**ข้อยกเว้นเดียวของขอบเขต "เฉพาะในวิชา" (decision 2026-08-06 · Release F):**
เพื่อเริ่ม Direct Message ผู้ใช้ค้นหาคนทั้งโรงเรียนได้ แต่**ค้นได้ ไม่ใช่ไล่ดูได้** —
ต้องพิมพ์คำค้นอย่างน้อย 3 ตัวอักษร ผลลัพธ์คืนแค่ชื่อ-นามสกุลกับรูปโปรไฟล์
ไม่มีหน้าที่ลิสต์สมาชิกทั้งโรงเรียน และไม่มีการเปิดเผย Identifier หรืออีเมลตามเดิม
...
หนึ่ง (Assignment / Material / Announcement) และหมดบทบาทเมื่อ entity นั้นเก่าไป
ส่วน Channel ไม่เกาะอะไรเลย มันคือที่คุยของวิชาที่อยู่ต่อเนื่องไม่ว่าจะมีโพสต์ใหม่หรือไม่
การมี Channel ไม่ได้ยกเลิก Comment — คำถามเรื่องการบ้านชิ้นหนึ่งยังควรอยู่ใต้การบ้านชิ้นนั้น

### Direct Message (DM)
บทสนทนาระหว่าง User สองคน **ไม่ผูกกับ CourseOffering** ผู้ใช้คนใดในโรงเรียนก็เริ่ม
บทสนทนากับใครก็ได้ (decision 2026-08-06) ต่างจาก Comment แบบ PRIVATE ซึ่งเป็น
ช่องครู↔นักเรียนที่เกิดใต้ Submission หนึ่งเท่านั้นและมีอยู่เพื่อพูดถึงงานชิ้นนั้น

### DM Privacy (ใครอ่าน DM ได้)
ไม่มีใครนอกจากคู่สนทนา — ไม่มีทั้งหน้าจอและสิทธิ์ให้ Admin หรือ Teacher เปิดอ่าน
บทสนทนาของคนอื่น การอ่านเกิดได้ทางเดียวคือ**มีคนกดรายงาน** ซึ่งจะ snapshot
เฉพาะข้อความที่ถูกรายงานพร้อมบริบทรอบ ๆ เข้า Moderation Case ผู้ตรวจเห็นแค่ snapshot นั้น
ไม่ใช่บทสนทนาทั้งเส้น (decision 2026-08-06)

### Block (การบล็อก)
ผู้ใช้บล็อกใครก็ได้ **รวมทั้ง Teacher** ผลคือส่ง DM หากันไม่ได้ทั้งสองทาง
และหายจากผลค้นหาของกันและกัน — เป็นการกระทำของเจ้าตัว ไม่ต้องรอผู้ใหญ่ตัดสิน

การบล็อก Teacher ไม่ตัดการสื่อสารเรื่องการเรียน เพราะ DM ไม่ใช่ช่องทางราชการ:
ประกาศ ฟีดวิชา การแจ้งเตือน และ Private Comment ใต้ Submission ยังทำงานตามเดิม
**ข้อบังคับที่ตามมา: ห้ามใช้ DM เป็นช่องทางสั่งการของโรงเรียน** ไม่อย่างนั้นการบล็อก
จะมีต้นทุนกับนักเรียนจนไม่มีใครกล้าใช้ (decision 2026-08-06)

### Message Preview (ตัวอย่างข้อความใน push)
Push ของ **ข้อความสนทนาเท่านั้น** (DM และ Channel) แสดงชื่อผู้ส่งและเนื้อหาได้
เป็นข้อยกเว้นที่ตั้งใจของ ADR-0047 และผู้ใช้ปิดได้เองต่ออุปกรณ์ (decision 2026-08-06)

**ข้อยกเว้นนี้ไม่ขยายไปไหนอีก** — คะแนน เกรด คำติชมของครู และเนื้อหางานที่ส่ง
ยังห้ามอยู่ใน push เหมือนเดิมทุกประการ เหตุผลของ ADR-0047 (แบนเนอร์บนหน้าจอล็อก
อ่านได้โดยคนที่นั่งข้าง ๆ) ยังใช้กับข้อมูลการเรียนเต็มร้อย

### Message Retention (อายุของข้อความ)
ข้อความแชท **ไม่ใช่หลักฐานทางการเรียน** ต่างจากคะแนน งานที่ส่ง และการเข้าเรียน
ซึ่งถูกเก็บถาวรใต้ตัวตนนิรนามเมื่อบัญชีถูก anonymize — ข้อความถูกลบอัตโนมัติเมื่ออายุ
เกิน 12 เดือน และถูกลบเมื่อเจ้าของถูก anonymize โดยเหลือ placeholder ให้บทสนทนา
อีกฝั่งยังอ่านรู้เรื่อง (decision 2026-08-06)

หลักฐานไม่หายไปกับมัน เพราะข้อความที่ถูกรายงานถูกคัดลอกเข้า
`ModerationCase.targetSnapshot` ซึ่งเป็นสำเนาแยกที่แก้ไม่ได้

### Directory Search (การค้นหาคน)
วิธีเดียวที่ใช้หาคู่สนทนา DM — ค้นด้วยชื่ออย่างน้อย 3 ตัวอักษร คืนชื่อ-นามสกุล
...
## Reward (Release F — V2 contract approved; V1 disabled on Production)

สัญญาปัจจุบันอยู่ใน [ADR-0056](./docs/adr/0056-course-score-milestones-and-system-quest-wallet.md)
และแบ่ง Reward เป็นสองระบบที่ไม่ใช้ยอดร่วมกัน

### Course Score Milestone (รางวัลตามคะแนนจริงของวิชา)

ครูเจ้าของวิชากำหนดของรางวัลและเกณฑ์ `Score Total` ตั้งแต่ 0–100 โดยใช้ค่าเดียวกับ
หน้าคะแนนนักเรียน: `Σ คะแนนที่ได้ ÷ Σ คะแนนเต็ม × 100` จาก Score Item ที่เผยแพร่แล้วเท่านั้น
ครูไม่ให้ “แต้มวิชา” ด้วยมืออีกต่อไป นักเรียนเห็นเส้นทางรางวัลและคะแนนรวมปัจจุบันของตนเอง

นักเรียนกดรับได้เฉพาะเกณฑ์สูงสุดที่ตนมีสิทธิ์และยังไม่เคยรับ ถ้าเคยรับเกณฑ์ 50 แล้วภายหลังถึง 80
ยังรับ 80 ได้ แต่ถ้าครั้งแรกเข้ามาตอนถึง 80 ระบบให้รับ 80 และทำเครื่องหมายเกณฑ์ต่ำกว่าที่ยังไม่รับเป็น
`SUPERSEDED` เพื่อไม่ให้ย้อนรับซ้อน คำขอต้องคำนวณคะแนนใหม่ฝั่ง server และเก็บ snapshot
ของคะแนนกับเวอร์ชันรางวัลไว้ตรวจสอบย้อนหลัง

คำขอเริ่มที่ `PENDING`; ครูเจ้าของวิชาเท่านั้นที่กด `FULFILLED` หรือ `REJECTED`
และการปฏิเสธต้องมีเหตุผล ของที่ส่งมอบแล้วไม่เรียกคืนเพราะการแก้คะแนนย้อนหลัง
รางวัลแต่ละเกณฑ์เป็นคำสัญญากับนักเรียนทุกคนที่มีสิทธิ์ จึงไม่ใช้ stock จำกัด

### System Quest Wallet (แต้มจากเควสระบบ)

Admin กำหนดเควสจากการกระทำที่ตรวจสอบได้และกำหนดแต้มแบบค่าคงที่ ระบบห้ามรวมคะแนนข้ามวิชา
ห้ามสุ่มแต้ม ห้าม reroll และห้ามกล่องสุ่ม แต้มเก็บเป็น immutable `SYSTEM` ledger
และใช้ได้เฉพาะร้านรางวัลของระบบ

ของรางวัลควรช่วยประสบการณ์ใน Beagle Classroom เช่น กรอบโปรไฟล์ badge/title ชุดมาสคอต
ธีม dashboard sticker/reaction และ template วางแผนการเรียน ห้ามของรางวัลเปลี่ยนคะแนน การเข้าเรียน
กำหนดส่ง สิทธิ์ส่งงาน การเข้าถึงวิชา ฟังก์ชันหลัก หรือ accessibility

`REWARD_ENABLED` และ `REWARD_MUTATIONS_ENABLED` ของ V1 ปิดอยู่บน Production
ระหว่างสร้าง V2 ส่วนข้อมูล ledger เดิมต้องเก็บไว้และห้ามแปลงหรือลบโดยอัตโนมัติ

## Reward V1 (historical contract — superseded by ADR-0056)
...
แต้มที่ครูเจ้าของวิชาตั้งกติกาเองและให้ของรางวัลเองภายในวิชานั้น แขวนอยู่กับ `Enrollment`
เหมือนคะแนนและการเข้าเรียน — ครูคนละคนตั้งอัตราต่างกันได้โดยไม่กระทบกัน

### System Quest (เควสของระบบ)
ภารกิจที่ Admin นิยามไว้ให้นักเรียนทำข้ามวิชา และ Admin เป็นผู้จัดของรางวัล
Admin นิยามเควสได้เพราะเควสไม่ใช่ข้อมูลการเรียนของใคร — ยังคงห้าม Admin ใส่คะแนน
เช็คชื่อ หรือสร้างการบ้านตามเดิม

**เควสของระบบนับได้เฉพาะการกระทำ ห้ามอ่านคะแนน** (decision 2026-08-06) —
"ส่งการบ้าน 3 ชิ้นสัปดาห์นี้" หรือ "เข้าเรียนครบ 2 สัปดาห์" ใช้ได้
"ได้คะแนนเกิน 5 ใน 3 วิชา" ใช้ไม่ได้ เพราะเป็นการรวมผลการเรียนข้ามวิชา
ซึ่ง § Aggregate GPA ถอดออกไปแล้ว และเพราะครูแต่ละคนตั้งความยากไม่เท่ากัน
เควสที่อิงคะแนนจึงไม่ยุติธรรมโดยโครงสร้าง เควสที่อิงคะแนนเป็นของครูภายในวิชาเท่านั้น

### Reward Ledger (บัญชีแต้ม)
แต้มถูกเก็บเป็น **รายการเดินบัญชี ไม่ใช่ยอดคงเหลือ** ยอดคือผลรวมของรายการ
เหตุผลเดียวกับที่ระบบนี้มี `AuditLog` และ `SubmissionVersion` แทนการเขียนทับ —
เด็กถามว่าแต้มหายไปไหนต้องตอบได้

**เมื่อข้อเท็จจริงเบื้องหลังเปลี่ยน ให้ออกรายการหักกลับ ไม่ใช่ลบรายการเดิม**
(decision 2026-08-06) ระบบนี้ให้ครูแก้เช็คชื่อและคะแนนย้อนหลังได้โดยออกแบบ
(§ Back-edit · `SCORE_EDIT_AFTER_PUBLISH`) แต้มที่อิงข้อเท็จจริงเหล่านั้นจึงต้องตามได้

**ของที่แลกไปแล้วไม่เรียกคืน** ต่อให้ยอดติดลบ — ความผิดพลาดของผู้ใหญ่ไม่ตกที่เด็ก

### Redemption (การแลกของ)
การแลกคือ **คำขอที่มีสถานะ ไม่ใช่ธุรกรรมที่จบในตัวเอง** (decision 2026-08-06)
เด็กกดแลก → รายการค้างอยู่ที่ครูหรือ Admin → มีการกดยืนยันว่าส่งมอบแล้ว
จนกว่าจะยืนยัน ทั้งสองฝั่งยังเห็นว่ารายการค้างอยู่

เพราะของรางวัลในโรงเรียนเกือบทั้งหมดอยู่นอกระบบ — สติกเกอร์ ขนม สิทธิ์เลือกที่นั่ง
ระบบส่งมอบแทนคนไม่ได้ ทำได้แค่จำว่าติดค้าง และของที่มีจำนวนจำกัดจะตัดสต๊อกตอนกดแลก
ไม่ใช่ให้กดได้ไม่จำกัดแล้วคัดทีหลัง

### Quest Randomness (การสุ่มเควส)
ทั้งชุดเควสและจำนวนแต้มที่ได้เป็นการสุ่ม (decision 2026-08-06)

**แต้มไหลทางเดียวเสมอ** — ได้มาจากการทำงานจริง ใช้ได้อย่างเดียวคือแลกของในร้าน
**ห้ามใช้แต้มสุ่มใหม่ ห้ามซื้อโอกาส ห้ามมีกล่องสุ่ม** เส้นนี้คือสิ่งที่แยก
"ของขวัญที่ไม่รู้ขนาดล่วงหน้า" ออกจากการพนัน และมันสำคัญเป็นพิเศษเพราะผู้ใช้เป็นเด็ก
ช่วงแต้มที่เป็นไปได้ต้องแสดงก่อนเริ่มทำ เช่น "เควสนี้ได้ 10–30 แต้ม" — สุ่มได้ แต่ไม่ปิดบัง

### Point Award (การจ่ายแต้ม)
**แต้มผูกกับความสำเร็จ ไม่ใช่การกระทำ — จ่ายครั้งเดียวต่อสิ่งที่สำเร็จ ต่อให้ทำซ้ำกี่รอบ**
(decision 2026-08-06) "ส่งการบ้านชิ้นนี้" จ่ายครั้งเดียวไม่ว่าจะมี `SubmissionVersion`
กี่อัน เพราะระบบนี้เก็บทุกเวอร์ชันโดยตั้งใจ ถ้าจ่ายต่อการกดส่งจะปั๊มได้ทันที
กติกานี้เข้ากับ § Reward Ledger พอดี เพราะรายการหักกลับยึดข้อเท็จจริงเดียวกัน

**ความเสี่ยงที่ยังเปิดอยู่ ยังไม่มีมาตรการ:** ครูตั้งอัตราแต้มเองได้โดยไม่มีเพดาน
ครูที่ให้แต้มสูงกว่าคนอื่นมากจะทำให้วิชาตัวเองกลายเป็นฟาร์ม และดึงเวลาเด็กไป
ตามอัตราแลกเปลี่ยนแทนความสำคัญของวิชา — รับรู้แล้วและเลือกที่จะยังไม่กัน

### Point Lifecycle (แต้มเมื่อความสัมพันธ์จบ)
**นักเรียนที่ถูกนำออกจากห้องจะถูกแช่แข็งแต้มไว้ ไม่ลบ แลกไม่ได้ และกลับมาใช้ได้
ถ้ากลับเข้าห้อง** (decision 2026-08-06) ระบบมี § Restore by Rejoin อยู่แล้ว
การเผาแต้มทิ้งทันทีจะทำให้การนำออกกลายเป็นบทลงโทษที่ไม่มีใครตั้งใจให้เป็น
และการนำออกอาจเป็นความผิดพลาด

**วิชาที่ถูก archive จะแช่แข็งระบบรางวัลทั้งหมด** — ยังดูบัญชีแต้มและประวัติการแลกได้
แต่ห้ามให้ หักกลับ ใช้แต้ม หรือยืนยันส่งมอบรางวัล จนกว่าครูจะ restore วิชา
แต้มไม่หมดอายุและไม่ถูกลบเพราะ archive (decision 2026-08-14)

**เมื่อบัญชีถูก anonymize ให้ยกเลิกรายการแลกที่ยังค้าง แล้วลบบัญชีแต้มและ
ประวัติการแลกทั้งหมด** เพราะรางวัลไม่ใช่หลักฐานทางการเรียนที่ต้องเก็บภายใต้
ตัวตนนิรนาม (decision 2026-08-14)

## Meeting (Release F design contract — ยังไม่ implement)

ภาษาส่วนนี้เป็นสัญญาการออกแบบที่ตกลงกันไว้ ยังไม่มีโค้ด schema หรือ route ใดรองรับ
เหตุผลเบื้องหลังอยู่ใน [ADR-0052](./docs/adr/0052-online-class-is-a-link-not-a-room.md)

### Meeting Link (ลิงก์ห้องเรียนออนไลน์)
ที่อยู่ของห้องเรียนออนไลน์ที่ครูใช้ — เป็น**ลิงก์ไปยังผู้ให้บริการภายนอก**
(Google Meet, Zoom) ระบบนี้ไม่สร้างห้องประชุมเอง ไม่มีเสียง ไม่มีภาพ

ลิงก์ประจำอยู่ที่ CourseOffering และ override รายคาบได้ที่ TimetableSlot ซึ่งมี
`location` (ห้องเรียนจริง) อยู่แล้ว — Meeting Link คือ `location` เวอร์ชันออนไลน์
ตอบคำถามเดียวกันว่า "คาบนี้เรียนที่ไหน" **ไม่เก็บที่ Session** เพราะ Session
สร้างแบบ lazy ตอนครูกดเปิดคาบ ลิงก์จึงจะยังไม่มีตอนเด็กเปิดดูตารางล่วงหน้า

ลิงก์เห็นได้เฉพาะ Enrollment ที่ active และครูเจ้าของวิชา — ลิงก์ที่หลุดออกไป
แปลว่าคนแปลกหน้าเข้าห้องเรียนเด็กได้ ไม่ใช่ metadata ธรรมดา

### Meeting Attendance (การเข้าห้องออนไลน์กับการเช็คชื่อ)
**ไม่มีผลต่อการเช็คชื่อ** ตราบใดที่ยังเก็บแค่ลิงก์ เพราะระบบไม่มีทางรู้ว่าใครเข้าห้อง

เมื่อใดที่ต่อ API ของผู้ให้บริการได้ **ระบบจะเป็นผู้เสนอ ไม่ใช่ผู้บันทึก**
(decision 2026-08-06) — หน้าเช็คชื่อแสดงว่าใครเข้าห้องและอยู่นานเท่าไร
พร้อมปุ่มให้ครูรับทั้งหมดในคลิกเดียว แต่แถว AttendanceRecord ยังเกิดจากครูเสมอ
เพราะ `EXCUSED` เดาจากการเข้าห้องไม่ได้ และ `markedById` ต้องไม่โกหกว่าใครตัดสิน
