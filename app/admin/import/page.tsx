import Link from "next/link";
import { Users, ChevronRight } from "lucide-react";

export default function AdminImportPage() {
  return (
    <div className="animate-fade-in space-y-5 p-6 md:p-10">
      <div>
        <h1
          className="text-3xl font-medium text-black md:text-4xl"
          style={{ letterSpacing: "-0.03em" }}
        >
          นำเข้าข้อมูล
        </h1>
        <p className="mt-1 text-sm text-black/60">
          เพิ่มข้อมูลจำนวนมากจากไฟล์ CSV
        </p>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        <Link
          href="/admin/import/teachers"
          className="card group p-6 hover:no-underline"
        >
          <div className="flex items-start justify-between">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-black/[0.05] text-black">
              <Users className="h-5 w-5" />
            </div>
            <ChevronRight className="h-5 w-5 text-black/20 transition-colors group-hover:text-black" />
          </div>
          <h2
            className="mt-4 font-medium text-black"
            style={{ letterSpacing: "-0.02em" }}
          >
            นำเข้าครู
          </h2>
          <p className="mt-1 text-sm text-black/60">
            อัพโหลด CSV ที่มี email, firstName, lastName — ระบบสร้างบัญชี +
            รหัสผ่านชั่วคราว
          </p>
        </Link>

        <div className="card-flat p-6 opacity-60">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-black/[0.04] text-black/40">
            <Users className="h-5 w-5" />
          </div>
          <h2
            className="mt-4 font-medium text-black"
            style={{ letterSpacing: "-0.02em" }}
          >
            นำเข้านักเรียน
          </h2>
          <p className="mt-1 text-sm text-black/60">
            นักเรียนสมัครเอง (Student self-register) — ไม่ต้อง CSV import
          </p>
        </div>
      </div>
    </div>
  );
}
