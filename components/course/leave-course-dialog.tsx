...
const INITIAL_STATE: LeaveCourseState = {};
...
export function LeaveCourseDialog({ courseId, courseName }: Props) {
  const router = useRouter();
  const dialogRef = useRef<HTMLDialogElement>(null);
  const [reason, setReason] = useState("");
  const [state, formAction] = useActionState(leaveCourseAction, INITIAL_STATE);
    ...
    <div className="card border-red-100 bg-red-50/40 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-semibold text-black">ออกจากวิชานี้</h2>
            <p className="mt-1 max-w-xl text-sm text-black/60">
              ใช้เมื่อจบภาคเรียน หรือไม่ต้องการเรียนใน Classroom นี้แล้ว งาน
              คะแนน และประวัติการเข้าเรียนเดิมจะยังถูกเก็บไว้
            </p>
          </div>
        </div>
        <button
      ...
      >
        <form action={formAction} className="p-6">
          ...
          <div className="rounded-xl bg-black/[0.04] p-3 text-xs text-black/70">
            หลังยืนยัน วิชานี้จะหายจาก Dashboard และรายการวิชาเรียนของคุณ
            แต่ข้อมูลเดิมยังคงอยู่ในระบบ
          </div>
...
function SubmitButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    ...
    >
      {pending ? "กำลังออกจากวิชา..." : "ยืนยันออกจากวิชา"}
    </button>
  );
}
