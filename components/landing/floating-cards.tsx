 ...
 * Classroom (ADR-0029 T1). Real product mini-cards (score, attendance,
 * feed post, reminder) float around the headline at different depths and
 * parallax-shift with the pointer. Each depth layer moves at a different
 * rate (data-depth) so the scene feels three-dimensional.
 *
 * Pointer parallax is applied to the whole layer via a single rAF-free
 * mousemove handler writing CSS vars; reduced-motion users get the
 * static composed layout (the handler simply never shifts).
 */
...
function FloatCard({
  ...
  // Stagger the idle bob by depth so the cards don't bob in unison.
  const bobDelay = (depth % 7) * 0.4;
  const bobDur = 5 + (depth % 5);
  return (
    <div
